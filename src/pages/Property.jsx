...
const Property = ({
  ...
  useEffect(() => {
    ...
    axios.get(`https://api.geoapify.com/v1/geocode/search?city=${locationData?.property_address?.city}&state=${locationData?.property_address?.state}&country=India&lang=en&limit=1&type=city&format=json&apiKey=${GEO_API}`)
    .then(
      (response) => {
  ...
  const locationItem = (
    ...
    >
      ...
      <div style={{ width: "100%", height: "300px" }}>
         {(cord.lat !== 0) &&  <GoogleMap lat={cord.lat} lng={cord.lng} zoom={13} />}
    </div>
    </div>
  );
